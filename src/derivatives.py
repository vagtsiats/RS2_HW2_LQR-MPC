...
def G(x):
    G = np.array(
        [
            [-g * m1 * l1 * np.sin(x[0, 0]) - g * m2 * (l1 * np.sin(x[0, 0]) + l2 * np.sin(x[0, 0] + x[1, 0]))],
...
def dC_dx(x):
    ...
    jac[:, :, 2] = np.array([[0, 0], [l1 * m2 * l2 * np.sin(q2), 0]])

    jac[:, :, 3] = np.array([[-2 * l1 * m2 * l2 * np.sin(q2), -l1 * m2 * l2 * np.sin(q2)], [0, 0]])
...
def dynamics_dx(x, u):
    ...
    jac = np.block([[dqdot_dx], [dqddot_dx]])
...
def rk4_dx(x, u, dt=0.05):
    ...
    jac = np.eye(4) + (dt / 6.0) * (f1 + 2.0 * f2 + 2.0 * f3 + f4)
    return jac
    ### END of TO_DO
...
def rk4_du(x, u, dt=0.05):
    ...
    jac = (dt / 6.0) * (f1 + 2.0 * f2 + 2.0 * f3 + f4)
    return jac
    ### END of TO-DO
